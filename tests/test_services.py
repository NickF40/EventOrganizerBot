...
def test_get_or_create_default_event_updates_capacity(tmp_path, monkeypatch):
    ...
    settings = config.Settings(telegram_token="token", event_name="Demo", attendee_limit=10)
...
def test_create_manual_attendee_creates_user_and_registration(tmp_path, monkeypatch):
    database, models, admin_service, events, _, _, _, _ = _reload_database(tmp_path, monkeypatch)
    settings = config.Settings(telegram_token="token", event_name="Demo", attendee_limit=None)
...
def test_register_user_waitlists_when_capacity_full(tmp_path, monkeypatch):
    database, models, _, events, _, _, registrations, _ = _reload_database(tmp_path, monkeypatch)
    settings = config.Settings(telegram_token="token", event_name="Demo", attendee_limit=1)
...
def test_schedule_and_query_posts(tmp_path, monkeypatch):
    ...
    with database.session_scope() as session:
        post = posts.schedule_post(session, title="Title", content="Body", send_at=send_at)
...
def test_broadcast_message_and_post(tmp_path, monkeypatch):
    database, models, _, _, messaging, posts, _, _ = _reload_database(tmp_path, monkeypatch)
    ...
    with database.session_scope() as session:
        session.add(models.User(telegram_id=1, username="ok", notifications_enabled=True))
        session.add(models.User(telegram_id=2, username="fail", notifications_enabled=True))
        session.add(models.User(telegram_id=None, username="skip", notifications_enabled=True))
        session.flush()
